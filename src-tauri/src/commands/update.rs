...
pub async fn update_device(window: Window, uuid: String) -> Result<String, String> {
  ...
  //     tx.send(payload).ok();
  // });
  let uuid = uuid.parse().map_err(|_| "error parsing UUID")?;

  let firmware: solo2::Firmware =
    solo2::Firmware::download_latest().map_err(|_| "could not download latest firmware")?;
  ...
  let device = solo2::Device::having(uuid).map_err(|_| "no such device")?;
  ...
  if let Ok(device) = device.into_lpc55() {
    ...
    let yes = false;
    Device::Lpc55(device)
      .program(firmware, yes, Some(&progress))
      .map(|_| calver)
    ...
    Err("Failed to switch to maintenance mode!".into())
  }
}
